...
Type TCompiler
	...
	Field Lines:String[]
	Field ModuleString:String
	
	Method LoadAllFunction(Path:String)
		Local dir%=ReadDir(CurrentDir()+Path)
		...
		Repeat
			...
			If t="" Exit
			If t="." Or t=".." Continue
			LoadFunction(CurrentDir()+Path+t)
		Forever
	...
	Method LoadFunction(FilePath:String)
		...
		For Local node:TxmlNode = EachIn Root.GetChildren()
			...
			Func.CodeName=Node.GetAttribute("name")
			Func.StringPtr = Int(Node.GetAttribute("stringptr"))
			If Node.GetChildren()
		...
		Next
	EndMethod
	Method SetModules(M:TList)
		For Local Modul:String=EachIn M
			ModuleString:+"~n"+LoadString(Modul)+"~n"
		Next
	EndMethod
		...
		For Local i:Int=0 Until Tmp.Length
			...
			Select Text
				Case "type"
					...
					Datatype.CanCastTo.AddLast(TDatatype.UnknownDatatype)
					...
					TDatatype.UnknownDatatype.CanCastTo.AddLast(Datatype)
			...
			Select Text
				...
				Case "function"
					...
					If SelTemplate'Nun den Namen mit dem überladenen Text ersetzen
						...
						While TmpLink
							If TmpLink._value And TTypeMethod(TmpLink._value) And TTypeMethod(TmpLink._value).Name=Id.Name
								If TTypeMethod(TmpLink._value).From=SelTemplate
									Id.GenName()
								EndIf
								If Not TmpLink._value DebugStop
						...
						Wend
						EndRem
						Local L:TList=SelTemplate.GetMethods(ScopeMode)
						For Local Meth:TTypeMethod=EachIn L
							If Meth.Name=Id.Name
								Meth.Func=Id
								Id.Owner=SelTemplate
								Meth.Name=String(Id.Name)
							EndIf
						Next
					Else
						Id.GenName()
	...
	Method Parser(DoDefs:Byte=True)
		...
		RootNode.AddAttribute("version","0.4")
		...
		If DoDefs
			For Local func:TIdentifierFunction=EachIn FunctionIdentifier
				...
				Node.AddAttribute("realname",Func. codeName)
				node.AddAttribute("userfunc", Func.UserFunc)
				Node.AddAttribute("stringptr",Func.Stringptr)
	...
	Method Keyword:TRecursive()
		Select GetCurrent()
			...
			Case "for"
				...
				Local N:TxmlNode=Variable.ToNode(Null)
				AddChild(TmpNode.AddChild("var"),N)
				If Not N.HasAttribute("type") N.AddAttribute("type", Variable.Datatype.Name)
			...
			Case "function"
				GetNext()
				If CurrentMode<>0 And CurrentMode<>3 Error("Function is in a scope!")
				Select GetCurrent()
					Case "virtual","override","abstract","static","final","setter","getter"
						GetNext()
				EndSelect
			...
			Case "return"
				...
				Local Rec:Trecursive=Expression()
				Node.AddAttribute("type",Rec.Datatype.Name)
			...
			Case "type"
				'DebugStop
				GetNext()
				If CurrentMode<>0 Error("Type is in a scope!")
				Local Name:String=GetCurrent()
				CurrentType=Name
				Local Datatype:TDatatype=New TDatatype
				...
				Repeat
					...
					If IsValidVarName(Name)
						...
						If IsToken("=") And DoSet
							...
							GetVariable(Name).Datatype.Add2Node(Node2)
		...
		For Local  Variable:TIdentifierVariable=EachIn GetVariableList()
			If GetCurrent().Trim().ToLower()=variable.Name.Trim().ToLower()
				...
				If GetCurrent()="=" And DoSet
					...
					EndIf
					Local Reca:TxmlNode=Node.AddChild("name")
					If RecAccess
						AddChild(Reca,RecAccess.Node)
					Else
						AddChild(Reca,Variable.ToNode(Positions))
					EndIf
					Rec.Datatype.Add2Node(Reca)
	...
	Method ParseType:TRecursive(RecIdent:TRecursive, Pos:TList)
		If GetCurrent()="."
			...
			Local LeftNode:TxmlNode=Node.AddChild("left")
			AddChild(LeftNode, RecIdent.node)
			RecIdent.Datatype.Add2Node(LeftNode)
			'DebugStop
			GetNext()
			Local Template:TTypeTemplate
			For Local Temp:TTypeTemplate=EachIn Templates
			...
			If GetCurrent()="."
				...
				If Rec.Info=Null
					...
					Return Rec
				Else
					Local TmpNode:TxmlNode=Node.AddChild("right")
					AddChild(TmpNode, Rec.Node)
					Rec.Datatype.Add2Node(TmpNode)
					Rec.Node=Node
					Return Rec
				EndIf
			Else
	...
	Method ValidDatatype:Byte(typ:String)
		If typ="int" Or typ="string" Or typ="float" Or typ="byte" Or Typ="long" Or Typ="short" Or Typ="double" Or Typ="unknown" Then Return True
...
Type TDatatype
	...
	Global UnknownDatatype:TDatatype
	...
	Function Init()
		...
		UnknownDatatype=New TDatatype 'beliebiger datentyp
		UnknownDatatype.Name="unknown"
		UnknownDatatype.Prio=-1
	...
	Function Create:TDatatype(Name:String, IsArray:Int=0)
		...
		Select Name
			...
			Case UnknownDatatype.Name.ToLower()
				Return UnknownDatatype.Copy(IsArray)
			Default
